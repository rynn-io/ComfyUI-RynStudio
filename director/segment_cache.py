...
def _segment_identity_fingerprint(seg: SegmentPlan, plan: DirectorPlan) -> dict[str, Any]:
    ...
    payload = {
        ...
        "ryn_segment_id": getattr(seg, "ryn_id", None),
        "ryn_model_loras": [
            {
                "id": entry.get("id"),
            ...
            for entry in getattr(plan, "ryn_applied_loras", ())
        ],
        "width": plan.width,
        "height": plan.height,
...
def segment_cache_fingerprint(seg: SegmentPlan, plan: DirectorPlan) -> dict[str, Any]:
    """Stable identity for an MVP segment; optional upstream processing is excluded."""
...
def _stamp_confirm_refine(result: dict[str, Any], plan: DirectorPlan) -> dict[str, Any]:
    ...
    """
    from .mvp_disabled import confirm_first_pass_enabled, refine_will_sample
