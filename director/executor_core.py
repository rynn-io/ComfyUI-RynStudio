...
def _segment_disk_cache_needed(
    ...
    *,
    ...
    if will_refine or hold_after_first:
        return True
    if face_refine_enabled(plan) or selflift_enabled(plan):
        return True
...
def execute_director_plan_core(
    ...
    *,
    ...
    def _run_one_segment(
        ...
        run_face_refine = face_refine_enabled(plan)
        pre_cache = (
        ...
        if run_face_refine and not hold_after_first:
            raise RuntimeError("FaceRefine is outside the Ryn H3 Director MVP")
        handoff = {
            "trim_frames": int(trim_frames),
