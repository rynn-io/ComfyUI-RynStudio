...
const HIDDEN_WIDGETS = [
    ...
    "ryn_state", "scene_id", "runtime_command",
...
class MiniMaxH3DirectorEditor {
    ...
    _writeTimelineWidget() {
        ...
        if (!isRynDirectorNode(this.node)) attachExternalGroupsWitness(this.node, payload);
        this.timelineWidget.value = JSON.stringify(payload);
        syncRynStateWidgets(this, payload);
...
function isMiniMaxH3DirectorNode(node) {
    const cls = node?.comfyClass || node?.type || "";
    return cls === "MiniMaxH3Director" || cls === "ComfyMiniMaxH3Director" || cls === "RynH3Director";
}
